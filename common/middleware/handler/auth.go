...
func LoginLogToDB(c *gin.Context, status string, msg string, username string) {
	...
	l["ipaddr"] = tools.GetClientIP(c)
	//fmt.Println("gaConfig.ExtConfig.AMap.Key", gaConfig.ExtConfig.AMap.Key)
	//l["loginLocation"] = tools.GetLocation(tools.GetClientIP(c), gaConfig.ExtConfig.AMap.Key)
	l["loginTime"] = tools.GetCurrentTime()
	l["status"] = status
	...
	q := runtime.App.GetMemoryQueue(c.Request.Host)
	message, err := runtime.App.GetStreamMessage("", global.LoginLog, l)
	if err != nil {
